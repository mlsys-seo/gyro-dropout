...
class AlexNet():
    ...
    def build_network(self, train_images, train_labels, eval_images, eval_labels):
        ...
        with tf.variable_scope("conv1"):
            ...
            pool2_eval = tf.nn.max_pool(conv2_eval, ksize=[1, 2, 2, 1],
                    strides=[1, 2, 2, 1], padding='SAME', name='pool2')

            with tf.variable_scope("conv3"):
                with tf.device('/gpu'):
                    kernel3 = tf.Variable(init([5, 5, 256, 384]))

            conv3 = tf.nn.relu(tf.nn.conv2d(pool2, kernel3, [1, 1, 1, 1], padding='SAME'))

            conv3_eval = tf.nn.relu(tf.nn.conv2d(pool2_eval, kernel3, [1, 1, 1, 1], padding='SAME'))

        with tf.variable_scope("fc1", reuse=tf.AUTO_REUSE):
            flatten_size = np.prod(conv3.get_shape().as_list()[1:])

            conv3_reshape = tf.reshape(conv3, [-1, flatten_size])
            conv3_reshape_eval = tf.reshape(conv3_eval, [-1, flatten_size])

            with tf.device('/gpu'):
                w1 = tf.Variable(init([conv3_reshape.get_shape().as_list()[1], self.fc_size]))
            layer1 = tf.nn.relu(tf.matmul(conv3_reshape, w1))
            ...
            layer1_eval = tf.nn.relu(tf.matmul(conv3_reshape_eval, w1))
